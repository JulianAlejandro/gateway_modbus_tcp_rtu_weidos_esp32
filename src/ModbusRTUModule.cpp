...
ModbusRTUModule::ModbusRTUModule(uint32_t baudrate) : _baudrate(baudrate) {}
...
bool ModbusRTUModule::readFromSlave(const modbusTCPStruct& request) {
...
int16_t ModbusRTUModule::readRegister() {
    return ModbusRTUClient.read();
}

const char* ModbusRTUModule::getLastError() {
